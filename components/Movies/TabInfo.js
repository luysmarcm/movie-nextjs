...
const TabInfo = ({ data, recommendations }) => {
	useEffect(() => {
	...
	return (
		<div className="rounded border-4 bg-black">
			<ul
				id="tabs"
				className="inline-flex pt-2 px-1 lg:w-full border-b overflow-hidden "
			>
				...
				<li className="px-4  font-semibold py-2 rounded-t">
					<a href="#third">Production Companies</a>
				</li>
				<li className="px-4  font-semibold py-2 rounded-t ">
			...
			<div id="tab-contents">
				...
				<div id="second" className="hidden p-4">
					<div className="flex mt-4">
						{data.credits.crew.slice(0, 4).map((crew, i) => (
				...
				<div id="third" className="hidden p-4">
					<div className="flex mt-4">
						{data.production_companies.slice(0, 5).map((pc, i) => (
							<div className="mr-8" key={i}>
								<div>{pc.name}</div>
							</div>
						))}
				...
				<div id="fourth" className="hidden p-4">
					<div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-8">
						{recommendations.results.slice(0, 5).map((movie, i) => (
