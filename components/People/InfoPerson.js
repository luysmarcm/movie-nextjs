...
const InfoPerson = ({data, age, knownFor}) => {
  return (
			...
			<div className="container mx-auto px-4 py-16 flex flex-col md:flex-row">
				<div className="flex-none w-72">
					<Image
						...
						alt={data.name}
						className="person-photo w-76 rounded-xl"
						id="person-photo"
						width={500}
						height={750}
						unoptimized
					...
					<ul className="flex items-center mt-4">
						...
						{data.external_ids.homepage && (
							<li className="ml-6">
								<a
									href={data.external_ids.homepage}
									title="Website"
									// target="_blank"
									rel="noopener"
								>
				...
				<div className="md:ml-24">
					...
					<div className="flex flex-wrap items-center text-gray-400 text-sm">
						...
						<span className="ml-2">
							{data.birthday} ({age} years old) in {data.place_of_birth}
						</span>
					</div>
					...
					<div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-8">
						{knownFor.map((movie, i) => (
							<div className="mt-4" key={i}>
								<Link passHref href={movie.linkToPage}>
									<Image
										loader={myLoader}
								...
								<Link href={movie.linkToPage} passHref>
									<a className="text-sm leading-normal block text-gray-400 hover:text-white mt-1">
										{movie.title}
									</a>
								</Link>
							</div>
